...
struct Camera {
  ...
  float fovy, aspect, near, far;
    ...
    rotate = glm::rotate(rotate, glm::radians<float>(this->pitch), glm::vec3(1.0, 0.0, 0.0));
...
int main(void)
{
  ...
  glGenBuffers(1, &water_indices);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, water_indices);
  ...
  Camera camera = { .position = glm::vec3(0.5, 0.5, 2.0),
	  .yaw = 0.0, .pitch = 0.0,
    .fovy = 90, 1260.0f / 1080.0f, 0.01, 1000.0
  };
  ...
  while (!window.should_close ())
  {
    ...
    glDrawElements(GL_TRIANGLES, sizeof(cube_indices) / 3.0f, GL_UNSIGNED_INT, (void *) water_indices);
